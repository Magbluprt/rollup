...
Don't print warnings to the console. If your configuration file contains an `onLog` or `onwarn` handler, this handler will still be called. The same goes for plugins with an `onLog` hook. To manually prevent that, you can access the command line options in your configuration file as described at the end of [Configuration Files](#configuration-files).
